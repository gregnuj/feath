...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=feath \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=feathd \
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/feathd

build: go.sum
ifeq ($(OS),Windows_NT)
	exit 1
else
	go build -mod=readonly $(BUILD_FLAGS) -o $(BUILDDIR)/feathd ./cmd/feathd
endif
...
build-reproducible-amd64: go.sum $(BUILDDIR)/
	$(DOCKER) buildx create --name feath-builder || true
	$(DOCKER) buildx use feath-builder
	$(DOCKER) buildx build \
		...
		--load \
		-f Dockerfile .
	$(DOCKER) rm -f feathd-binary || true
	$(DOCKER) create -ti --name feathd-binary feath:local-amd64
	$(DOCKER) cp feathd-binary:/usr/local/bin/feathd $(BUILDDIR)/feathd-linux-amd64
	$(DOCKER) rm -f feathd-binary
...
build-reproducible-arm64: go.sum $(BUILDDIR)/
	...
	$(DOCKER) buildx build \
		...
		-f Dockerfile .
	$(DOCKER) rm -f feathd-binary || true
	$(DOCKER) create -ti --name feathd-binary feath:local-arm64
	$(DOCKER) cp feathd-binary:/usr/local/bin/feathd $(BUILDDIR)/feathd-linux-arm64
	$(DOCKER) rm -f feathd-binary
...
draw-deps:
	...
	@goviz -i ./cmd/feathd -d 2 | dot -Tpng -o dependency-graph.png
