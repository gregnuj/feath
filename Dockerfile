...
RUN --mount=type=cache,target=/root/.cache/go-build \
    ...
    go install \
        ...
        -ldflags " \
            ...
            -X github.com/cosmos/cosmos-sdk/version.Name='feath' \
...
FROM alpine:${ALPINE_VERSION} as feath

COPY --from=app-builder /go/bin/feathd /usr/local/bin/feathd

WORKDIR /feath

CMD ["/usr/local/bin/feathd", "--home", "/feath", "start"]